...
void write_data_chunk(
    ...
    const uint64_t chunk_size_in_bytes_uint64 =
        static_cast<uint64_t>(chunk_size_in_bytes);
    ...
    file.write(
        reinterpret_cast<const char*>(&chunk_size_in_bytes_uint64),
        sizeof(uint64_t)
    );
    file.write(
...
void write_data_chunk_body(
    std::ofstream& file,
    const size_t size_in_bytes_to_write,
    const void* const data
) {
    file.write(
        reinterpret_cast<const char*>(data),
        size_in_bytes_to_write
    );
}
